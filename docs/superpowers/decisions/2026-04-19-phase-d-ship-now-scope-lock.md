...
- Screenshots saved to `docs/qa-evidence/phase-d-ship-now-qa/` (archived from Playwright MCP scratch after the QA run).
...
- **2026-04-20 — Plan C shipped.** Commit series `2c3f00a..ce600e2` on `feature/phase-d-plan-c-backend-hygiene`, fast-forward merged to `main` and pushed. Symmetric 4-layer removal of the legacy `confidence` scalar complete across frontend types, frontend read site (OutcomesView denominator also corrected 5→10), backend response serializers (`_serialize_decision` + intelligence outcomes), and backend write path (`FridayDecisionCreateRequest.confidence_vs_spy_riskadj` now required; `add_decision` legacy kwarg + mutual-exclusion branch removed). Discord/Telegram cron success message now conditionally echoes the latest `WeeklySnapshot.comment` via a new `latest_comment` parameter on `send_cron_success` (None/empty/whitespace-safe). Final state: 72/72 backend tests passing, frontend tsc clean. Executed via subagent-driven-development: 5 tasks × {implementer + spec review + code quality review}. Playwright MCP QA per §Playwright MCP QA is the next remaining Ship-Now item.
- **2026-04-20 — Playwright MCP QA result: 11/13 PASS, 2 SKIPPED, zero regressions.** Screenshots at `docs/qa-evidence/phase-d-ship-now-qa/` (7 files, archived from Playwright MCP scratch). No hotfix commits required.
  - **PASS (11):** Items 1 (no Bell), 2 (A1 briefing with user's 2026-04-17 comment), 3 (A2 Sleeve Health × 6 sleeves incl. `BONDS-CASH` canonical), 4 (no hero/delta/signals/macro/journal/archive regressions), 5 (3 stacked sliders with `primary/baseline/stretch` anchors + readouts), 6 (`Expected #1 ≥ #2 ≥ #3 — your ordering deviates. Logged for calibration.` at riskadj=3, cash=8 in muted italic, screenshot `06-ordering-deviation.png`), 7 (5-option failure-mode dropdown + trigger-threshold numeric input), 8 (`💬 이번 주 코멘트` details expands textarea with Korean placeholder), 10 (archive italic left-accent `WEEKLY COMMENT` quote at top), 11 (`Conf 10` primary + `vs Cash 10 · vs SPY Pure 6` secondary; `Mode:`/`Threshold:`/invalidation line correctly absent because 2026-04-17 is a pre-A4 freeze with `None` values — not a regression), 13 (`GET /api/v1/friday/snapshot/2026-04-17` read-path: decision payload keys are exactly `[assetTicker, confidenceVsCash, confidenceVsSpyPure, confidenceVsSpyRiskadj, createdAt, decisionType, expectedFailureMode, id, invalidation, note, snapshotId, triggerThreshold]` — **legacy `confidence` key absent** per Plan C).
  - **SKIPPED (2):** Items 9 (interactive freeze + step indicator) and 12 (POST request body inspection). Reason: `backend/.env` routes local uvicorn to **prod Supabase**, so any freeze/decision POST would pollute prod archive; a sqlite fallback was attempted but blocked by postgres-only `JSONB` columns on `weekly_snapshots.frozen_report` (and peers) when running `Base.metadata.create_all()`; the current week (2026-04-17) is already frozen on prod so idempotency would also block a fresh freeze. Recommended follow-up: either add a sqlite-compatible `JSONB`→`JSON` type decorator for test mode, or stand up a disposable postgres for integration QA. Plan C write-path coverage is already in the 72/72 backend pytest suite, so this is a `ship-risk: low` gap.
  - **Prod smoke (Vercel `https://portfolio-tracker-omega-nine.vercel.app/friday`):** `/friday` + `/friday/2026-04-17` both render cleanly; no Bell in header; A1 briefing surfaces user's 2026-04-17 comment; A2 Sleeve Health renders 6 sleeves incl. normalized `BONDS-CASH`; archive card shows `Conf N` primary (filled from the rename migration of the old `confidence` column). Zero DevTools Console errors on both pages.
