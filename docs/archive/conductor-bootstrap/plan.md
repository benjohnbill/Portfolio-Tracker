...
Archived from `conductor/plan.md` during control-plane reduction.

This document served as the project's state machine. Tools (MCPs) were strictly bound to specific phases.
...
## Phase 2: Real-time Data & Dashboard (Complete Foundation)
...
**Detailed Plan:** See `phase4-quant-engine.md` for Maestro agent breakdown.
...
## Phase 5: ELT Data Pipeline & Zero-Latency UI (Current Focus at time of archival)
**Detailed Plan:** See `phase5-elt-pipeline.md` for Maestro agent breakdown.

### Phase 5.1: Database Schema for ELT
- [x] Task 1: Create `raw_daily_prices` table.
- [x] Task 2: Create `portfolio_snapshots` table for cached equity curve.

### Phase 5.2: The 'Extract & Load' Worker
- [x] Task 1: Implement daily cron job to fetch missing raw prices and dump to DB.

### Phase 5.3: The 'Transform' Engine
- [x] Task 1: Refactor `PortfolioService` to calculate equity curve from DB raw prices.
- [x] Task 2: Save final daily portfolio snapshot to `portfolio_snapshots`.

### Phase 5.4: 0.1s API Refactor
- [x] Task 1: Refactor `/api/portfolio/history` to only SELECT from `portfolio_snapshots`.
