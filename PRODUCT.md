...
- **Phase W — Weekly-review native integration.** The `/weekly-review` skill currently stores its synthesis output as a local markdown file (`docs/weekly-reports/`) with no app-side surface. Two gaps: (1) the skill was authored against the legacy 40/35/25 scoring model and must be updated to reflect v2.4 30/30/40 — Posture as primary risk signal (40 pts), Posture < 8 veto check made explicit in Section 5A; (2) the Phase 2 synthesis report should be persisted to a `weekly_analysis_reports` DB table via `POST /api/intelligence/weekly-analysis` and read back by `/intelligence/reviews`, enabling in-app access to full historical analyses, DB-based 12-week calibration (replaces the unbuilt corpus pipeline), and the §9 error-memory and trust-calibration accumulation axes. The Friday Dashboard form remains the entry point for confidence scores and invalidation (no change); only the rich report body gains a DB home.
