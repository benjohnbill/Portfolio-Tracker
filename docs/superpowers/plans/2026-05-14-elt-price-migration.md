...
| 3 | Sync/async backfill | Sync direct call. No timeout — bounded by yfinance's own response time (typically 1-3s for 3y of daily closes). On exception, log warning and continue with transaction creation; daily cron picks up the gap. | Originally specced as `5s timeout via ThreadPoolExecutor` — but `with ThreadPoolExecutor()`'s shutdown(wait=True) blocks the main thread until the worker finishes regardless of `future.result(timeout=)`, so the budget was unenforceable. Honest sync aligns with ELT paradigm: yfinance only on write path. Revised 2026-05-14 after Phase 2 review. |
