...
|---|---|---|---|---|---|
| **MacroVitalsWidget** | `/api/macro-vitals` | 8113 / 8534 | **EXCEPTION** | FAST (cache hit) after Task 1.1 deploy | `/` (dashboard) |
| **StressTestWidget** | `/api/stress-test` | 2290 / 2170 | **SLOW** | MEDIUM after Task 1.3 (batch query) + 1.2 (price cache) | `/` (dashboard) |
| **EquityCurveSection** | `/api/portfolio/history?period=…` | 738 / 1493 | MEDIUM (variance — see note) | MEDIUM unchanged (no fix in plan) | `/portfolio` |
| **MSTRSignalSection** | `/api/signals/mstr-history?period=…` | 939 / 867 | MEDIUM | MEDIUM unchanged | `/portfolio` |
| **AssetSignalSection (QQQ)** | `/api/signals/history?ticker=QQQ` | 690 / 557 | MEDIUM | MEDIUM unchanged | `/portfolio` |
| **AssetSignalSection (GLDM)** | `/api/signals/history?ticker=GLDM` | 588 / 558 | MEDIUM | MEDIUM unchanged | `/portfolio` |
| **AssetSignalSection (TLT)** | `/api/signals/history?ticker=TLT` | 523 / 717 | MEDIUM | MEDIUM unchanged | `/portfolio` |
| **PortfolioSummaryCard** | `/api/portfolio/summary` | 382 / 373 | **FAST** | FAST | `/portfolio` |
| **AssetAllocationSection** | `/api/portfolio/allocation` | 364 / 372 | **FAST** | FAST | `/portfolio` |
| **MacroContextSection** | `/api/macro-context` (24h ISR via `unstable_cache`) | served from Next.js data cache (sub-ms after first build) | **FAST** (after ISR warm) / SLOW (cold ISR — falls back to macro-vitals path) | FAST after Task 1.1 (cold ISR also fast) | `/friday`, `/intelligence` |
| **FridayReportSection** | `/api/v1/friday/current`, `/api/v1/friday/snapshots`, conditional `/api/v1/friday/snapshot/<date>` | not directly measured (Friday page total: 1804ms warm) | MEDIUM | MEDIUM | `/friday` |
| **FridayBriefingSection** | `/api/v1/friday/briefing` | not directly measured | MEDIUM | MEDIUM | `/friday` |
| **IntelligenceSection (Hero/Stats/Heatmap/Decisions)** | `/api/intelligence/*` family | not directly measured (Intelligence page total: 1140ms warm) | FAST | FAST | `/intelligence` |
...
| Post-fix measurement (deploy required) | pending Task 4.1 — fills the "Post-fix expectation" column with measured numbers and confirms the EquityCurve variance fix is or isn't needed |
